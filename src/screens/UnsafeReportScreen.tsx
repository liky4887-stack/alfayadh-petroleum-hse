...
export default function UnsafeReportScreen({ navigation }: { navigation: NativeStackNavigationProp<RootStackParamList> }) {
  ...
  const handleLocation = async () => {
    ...
    try {
      ...
      setLocation({ lat: pos.coords.latitude, lng: pos.coords.longitude }); haptics.notificationSuccess();
  ...
  const canProceed = () => { if (step === 1) return note.trim().length > 0; if (step === 2) return department !== null; if (step === 3) return subcategory !== null; return true; };
  ...
  const handleSubmit = async () => {
    if (!classification || !note.trim() || !department || !subcategory || !status) { haptics.notificationError(); Alert.alert('تنبيه', 'يرجى إكمال جميع الحقول المطلوبة'); return; }
    setSubmitting(true);
    let imageUrl: string | null = null;
    if (imageUri) { imageUrl = await uploadImage(imageUri, 'reports'); }
    const result = await submitReport({ type: classification, note: note.trim(), corrective_action: correctiveAction.trim() || null, image_url: imageUrl, department, subcategory, status, location_lat: location?.lat ?? null, location_lng: location?.lng ?? null });
    setSubmitting(false);
    if (result.success) {
      if (status === 'open') { haptics.notificationError(); Alert.alert('تنبيه: تقرير مفتوح', 'تم تسجيل التقرير كحالة مفتوحة تحتاج تدخلاً. تم إرسال إشعار للجهات المعنية.', [{ text: 'حسناً', onPress: () => navigation.navigate('Dashboard') }]); }
      else { haptics.notificationSuccess(); Alert.alert('تم الإرسال', result.offline ? 'تم حفظ التقرير محلياً. سيتم المزامنة عند عودة الاتصال.' : 'تم توثيق التقرير وإغلاقه.', [{ text: 'حسناً', onPress: () => navigation.navigate('Dashboard') }]); }
    } else { haptics.notificationError(); Alert.alert('خطأ', 'تعذر إرسال التقرير. يرجى المحاولة مرة أخرى.'); }
  };
      ...
      <ScrollView style={S.scroll}>
        <View style={S.body}>
          <View style={S.progressRow}>
            {Array.from({ length: TOTAL_STEPS }).map((_, i) => (<View key={i} style={[S.progressBar, { backgroundColor: i <= step ? Dark.emerald : Dark.graphite }]} />))}
          </View>
          ...
          {step === 0 && (
            <View style={S.stepContent}>
              ...
              <Pressable onPress={() => handleClassification('unsafe_condition')} style={({ pressed }) => [S.classifyBtn, { borderColor: Dark.red }, pressed && S.pressed]}>
                ...
                <View style={S.classifyText}><Text style={S.classifyTitleRed}>حالة غير آمنة</Text><Text style={S.classifySub}>ظرف أو وضع مادي يشكل خطراً</Text></View>
              </Pressable>
              <Pressable onPress={() => handleClassification('unsafe_act')} style={({ pressed }) => [S.classifyBtn, { borderColor: Dark.red }, pressed && S.pressed]}>
                ...
                <View style={S.classifyText}><Text style={S.classifyTitleRed}>تصرف غير آمن</Text><Text style={S.classifySub}>سلوك بشري يخالف إجراءات السلامة</Text></View>
              </Pressable>
            </View>
          )}
          {step === 1 && (
            <View style={S.stepContent}>
              <View style={S.field}><Text style={S.fieldLabel}>الملاحظة *</Text><TextInput value={note} onChangeText={setNote} placeholder="صف الحالة أو التصرف غير الآمن بالتفصيل..." multiline numberOfLines={4} /></View>
              <View style={S.field}><Text style={S.fieldLabel}>الإجراء التصحيحي</Text><TextInput value={correctiveAction} onChangeText={setCorrectiveAction} placeholder="ما الإجراء المتخذ أو المطلوب؟" multiline numberOfLines={3} /></View>
              <View style={S.rowBtns}>
                <Pressable onPress={handleImage} style={({ pressed }) => [S.toggleBtn, { borderColor: imageUri ? Dark.emerald : Dark.graphite, backgroundColor: imageUri ? Dark.emeraldBg : Dark.slate }, pressed && S.pressed]}>
                  <Camera size={18} color={imageUri ? Dark.emerald : Dark.steel} /><Text style={[S.toggleText, { color: imageUri ? Dark.emerald : Dark.steel }]}>{imageUri ? 'تم الإرفاق' : 'إرفاق صورة'}</Text>
                </Pressable>
                <Pressable onPress={handleLocation} style={({ pressed }) => [S.toggleBtn, { borderColor: location ? Dark.emerald : Dark.graphite, backgroundColor: location ? Dark.emeraldBg : Dark.slate }, pressed && S.pressed]}>
                  <MapPin size={18} color={location ? Dark.emerald : Dark.steel} /><Text style={[S.toggleText, { color: location ? Dark.emerald : Dark.steel }]}>{location ? 'تم التحديد' : 'تحديد الموقع'}</Text>
                </Pressable>
              </View>
              {imageUri && (<View style={S.imagePreviewWrap}><Image source={{ uri: imageUri }} style={S.imagePreview} /><Pressable onPress={() => { haptics.impactMedium(); setImageUri(null); }} style={S.removeImageBtn}><X size={16} color="#FFF" strokeWidth={2.5} /></Pressable></View>)}
              {location && (<Text style={S.coords}>الإحداثيات: {location.lat.toFixed(4)}, {location.lng.toFixed(4)}</Text>)}
            </View>
          )}
          {step === 2 && (<View style={S.stepContent}><Text style={S.stepTitle}>اختر القسم المسؤول</Text><View style={S.chipWrap}>{DEPARTMENT_KEYS.map((dept) => (<Chip key={dept} label={dept} selected={department === dept} onPress={() => handleDepartment(dept)} />))}</View></View>)}
          {step === 3 && (<View style={S.stepContent}><Text style={S.stepTitle}>التصنيف الفرعي — {department}</Text>{department && DEPARTMENTS[department] && (<View style={S.chipWrap}>{DEPARTMENTS[department].map((sub) => (<Chip key={sub} label={sub} selected={subcategory === sub} onPress={() => handleSubcategory(sub)} />))}</View>)}</View>)}
          {step === 4 && (
            <View style={S.stepContent}>
              ...
              <Pressable onPress={() => handleStatus('closed')} style={({ pressed }) => [S.statusBtn, { borderColor: status === 'closed' ? Dark.green : Dark.graphite, backgroundColor: status === 'closed' ? Dark.greenBg : Dark.slate }, pressed && S.pressed]}>
                <Text style={[S.statusTitle, { color: status === 'closed' ? Dark.green : Dark.offWhite }]}>مغلقة (تم الحل ميدانياً)</Text><Text style={S.statusSub}>تم اتخاذ الإجراء اللازم وتم الحل في الموقع</Text>
              </Pressable>
              <Pressable onPress={() => handleStatus('open')} style={({ pressed }) => [S.statusBtn, { borderColor: status === 'open' ? Dark.red : Dark.graphite, backgroundColor: status === 'open' ? Dark.redBg : Dark.slate }, pressed && S.pressed]}>
                <Text style={[S.statusTitle, { color: status === 'open' ? Dark.red : Dark.offWhite }]}>مفتوحة (تحتاج تدخل)</Text><Text style={S.statusSub}>الحالة مستمرة وتتطلب إجراءً من الجهة المعنية</Text>
              </Pressable>
            </View>
          )}
          <View style={S.navBtns}>
            <Pressable onPress={handleBack} style={({ pressed }) => [S.backBtn, pressed && S.pressed]}><ChevronLeft size={20} color={Dark.steel} /><Text style={S.backText}>{step === 0 ? 'إلغاء' : 'السابق'}</Text></Pressable>
            <View style={{ flex: 1 }} />
            {step < TOTAL_STEPS - 1 ? (<Button label="التالي" onPress={handleNext} variant="primary" disabled={!canProceed()} />) : (<Button label={submitting ? 'جاري الإرسال...' : 'إرسال التقرير'} onPress={handleSubmit} variant="danger" disabled={submitting || !status} />)}
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
